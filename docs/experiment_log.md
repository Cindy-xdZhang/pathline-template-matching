...
| `mainExp_TemplateMatching_1.2` | 2026-08-27 | DEVELOPMENT_PROTOCOL_FROZEN_NOT_RUN | 继承1.1；唯一修订是空类 library stratum 两类模板都选0并审计，query/PCA-fit/prior不删除该数据 | `config/mainExp_TemplateMatching_1.2_development.yaml`, `docs/mainExp_TemplateMatching_1.2.md` | 与1.1相同的 exposed cache、7-family leave-one-out；sealed confirmation access forbidden | 与1.1相同；另报 skipped stratum/candidate counts | 尚未运行、无性能结论；该版本来自1.1的 pre-metric failure，不使用任何性能结果作修改依据 |
