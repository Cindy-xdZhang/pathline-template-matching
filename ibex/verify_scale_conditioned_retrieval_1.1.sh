...
OUTER_FAMILY=${FAMILIES[$SLURM_ARRAY_TASK_ID]}
...
JOB_TMP_ROOT="${SLURM_TMPDIR:-/tmp}/ptm_scale_nested_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
export NUMBA_CACHE_DIR="$JOB_TMP_ROOT/numba_cache"
mkdir -p "$NUMBA_CACHE_DIR"

COMMIT_ID=$(git rev-parse HEAD)
SHORT_COMMIT=$(git rev-parse --short=12 HEAD)
RUN_DIR="$EXPERIMENT_ROOT/runs/slurm_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}_${SHORT_COMMIT}_outer_${OUTER_FAMILY}"

echo "experiment=Verify_ScaleConditionedRetrieval_1.1"
echo "phase=exposed_train_only_nested_family_validation"
