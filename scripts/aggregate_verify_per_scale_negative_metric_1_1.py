#!/usr/bin/env python3
"""Authenticate and aggregate Verify_PerScaleNegativeMetric_1.1 folds.
...
AGGREGATE_SUMMARY_SCHEMA = (
    "pathline_template_matching.per_scale_negative_metric_aggregate_summary.v1"
)
SINGLE_FOLD_REPORT_SCHEMA = (
    "pathline_template_matching.per_scale_negative_metric_single_fold_authentication_report.v1"
)
EARLY_STOP_CERTIFICATE_SCHEMA = (
    "pathline_template_matching.per_scale_negative_metric_early_stop_certificate.v1"
)
AGGREGATE_MANIFEST_SCHEMA = (
    "pathline_template_matching.per_scale_negative_metric_aggregate_manifest.v1"
)
AGGREGATE_COMPLETE_SCHEMA = (
    "pathline_template_matching.per_scale_negative_metric_aggregate_complete.v1"
)
...
def _authenticate_fold(
    ...
    *,
    ...
    scaler = runner.authenticate_and_rebuild_final_scaler(
        fold_path / "final_per_scale_scaler.npz",
        fold_path / "final_per_scale_scaler_manifest.json",
        plan=plan,
        selected=candidate,
        outer_family=family,
        ...
        expected_manifest_file_sha256=str(
            result["final_scaler_manifest_file_sha256"]
        ),
    )
    _require(
        scaler.scaler_file_sha256 == result["final_scaler_file_sha256"],
        f"{fold_path}: authenticated scaler file SHA drifted",
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default=str(ROOT / "config" / "Verify_PerScaleNegativeMetric_1.1.yaml"),
    )
