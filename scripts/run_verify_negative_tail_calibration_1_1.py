#!/usr/bin/env python3
"""Frozen nested-family runner for Verify_NegativeTailCalibration_1.1.
...
CALIBRATION_ARTIFACT_SCHEMA = (
    "pathline_template_matching.negative_tail_calibration_artifact.v1"
)
CALIBRATION_MANIFEST_SCHEMA = (
    "pathline_template_matching.negative_tail_calibration_manifest.v1"
)
PREDICTION_SCHEMA = "pathline_template_matching.negative_tail_outer_prediction.v1"
PREDICTION_MANIFEST_SCHEMA = (
    "pathline_template_matching.negative_tail_outer_prediction_manifest.v1"
)
...
def load_plan(config_path: str | Path) -> Plan:
    ...
    _require(raw.get("status") == "frozen_pre_run_not_run", "frozen status drifted")
    ...
    _require(retrieval.get("distance") == "exact_euclidean", "distance drifted")
    _require(calibration.get("reference_population") == "fit_family_natural_negative_rows_only", "calibration reference drifted")
    _require(calibration.get("leave_one_out_definition") == "explicit_self_row_exclusion_after_one_global_negative_scaler_fit", "leave-one-out definition drifted")
    ...
    _require(len(required) == 13 and len(set(required)) == 13, "required fold files drifted")
...
def authenticate_and_rebuild_final_calibration(
    ...
    *,
    ...
    rebuilt = ScaleConditionedNegativeTailCalibrator.from_arrays(arrays)
    _require(rebuilt.ks == (selected.k,), "rebuilt final calibrator k drifted")
    _require(
        _json_safe(rebuilt.fit_audit) == manifest.get("fit_audit"),
...
def _authenticate_inner_selection_evidence(
    *,
    ...
    _require(audits.get("schema") == "pathline_template_matching.negative_tail_inner_fit_audits.v1", "inner fit audit schema drifted")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default=str(ROOT / "config" / "Verify_NegativeTailCalibration_1.1.yaml"))
